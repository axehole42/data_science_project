...
for feat in all_features:
    ...
    try:
        ...
        stats_list.append({'Feature': feat.replace('_', '\\_'), 'N': '---', 'MissPct': '---', 'Size_p': '---', 'Nested_p': '---', 'Mechanism': 'Error'})

# LaTeX Generation
print(r"\begin{table}[h]")
print(r"\centering")
print(r"\scriptsize")
print(r"\caption{Formal Rubin Test: Diagnosis via Baseline Size and Incremental Nested LRT}")
print(r"\label{tab:rubin_test_nested_bulletproof}")
print(r"\begin{tabular}{lrrcccl}")
print(r"\toprule")
print(r"\textbf{Feature} & \textbf{N} & \textbf{Miss %} & \textbf{Size p-val} & \textbf{Nested LRT p-val} & \textbf{Conclusion} \\")
print(r"\midrule")

for row in stats_list:
    line = f"{row['Feature']} & {row['N']} & {row['MissPct']} & {row['Size_p']} & {row['Nested_p']} & {row['Mechanism']} \\\\"
    print(line)

print(r"\bottomrule")
print(r"\multicolumn{6}{l}{\footnotesize \textit{Note: Size p-val tests baseline missingness. Nested LRT p-val tests incremental Target power. $p < 0.05$ used for MAR.}} \\")
print(r"\end{tabular}")
print(r"\end{table}")